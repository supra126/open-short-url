...
export class UrlService implements OnModuleInit {
  ...
  // ==================== URL Count Management (for UrlBulkService) ====================

  /**
   * Decrement URL count (used by UrlBulkService for bulk delete operations)
   */
  decrementUrlCount(count: number): void {
    this.urlCount = Math.max(0, this.urlCount - count);
  }

  /**
   * Get current URL count (for monitoring/debugging)
   */
  getUrlCount(): number {
