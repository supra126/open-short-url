...
OG 圖片上傳（社群預覽）功能所需。支援 AWS S3、Cloudflare R2 和 MinIO。

| 變數                   | 說明                 | 預設值 |
| ---------------------- | -------------------- | ------ |
| `S3_REGION`            | S3 區域              | `auto` |
| `S3_ENDPOINT`          | S3 相容端點 URL      | -      |
| `S3_ACCESS_KEY_ID`     | 存取金鑰 ID          | -      |
| `S3_SECRET_ACCESS_KEY` | 秘密存取金鑰         | -      |
| `S3_BUCKET`            | 儲存桶名稱           | -      |
| `S3_PUBLIC_URL`        | CDN 公開 URL（選填） | -      |
| `S3_GLOBAL_PREFIX`     | 全域金鑰前綴（選填） | -      |
